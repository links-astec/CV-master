  ...
  <div class="step-wrap">
    ...
    <div v-if="photoTemplates.includes(store.template)" class="photo-section" :class="{ 'photo-required': store.template === 'photo' }">
      ...
      <div class="photo-upload-row">
        ...
        <div class="photo-drop-zone" :class="{ dragging }" @click="triggerPick" @dragover.prevent="dragging=true" @dragleave="dragging=false" @drop.prevent="handleDrop">
          ...
          <div v-if="!store.data.photo" class="dz-content">
            ...
            <div class="dz-hint">JPG, PNG or WEBP · Max 5MB · Headshot style</div>
          </div>
          <div v-else class="dz-content">
            <svg class="dz-icon" viewBox="0 0 24 24" fill="none" stroke="var(--c-green)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            <div class="dz-primary" style="color:var(--c-green);">Photo uploaded</div>
            <div class="dz-hint">Click or drop to replace</div>
          </div>
        </div>
      </div>
      <div v-if="store.data.photo" class="photo-actions">
        <button class="photo-rm-btn" @click="store.data.photo = null">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="width:13px;height:13px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M9 6V4h6v2"/></svg>
          Remove photo
        </button>
      </div>
    ...
    <div class="f-row">
      ...
      <div class="f-grp">
        <div class="f-lbl">LinkedIn URL <span class="opt-lbl">optional</span></div>
        <input class="f-inp" v-model="store.data.li" placeholder="linkedin.com/in/you" autocomplete="off" />
      </div>
    </div>
    <div class="f-grp">
      <div class="f-lbl">Website / Portfolio <span class="opt-lbl">optional</span></div>
...
function handleDrop(e) { dragging.value = false; processFile(e.dataTransfer.files[0]) }
...
.photo-section-lbl { display:flex;align-items:center;gap:7px;font-size:12px;font-weight:600;color:var(--c-text);margin-bottom:14px; }
.photo-section-lbl svg { width:14px;height:14px;color:var(--c-text2); }
.req-badge { margin-left:4px;font-size:9.5px;font-weight:700;color:var(--c-accent);background:#fff;padding:2px 7px;border-radius:20px;border:1px solid var(--c-accent); }
...
.photo-circle { width:72px;height:72px;border-radius:50%;flex-shrink:0;overflow:hidden;cursor:pointer;position:relative;background:var(--c-bg);border:2px dashed var(--c-border2);transition:border-color .18s; }
...
.photo-overlay { position:absolute;inset:0;background:rgba(42,91,215,.65);display:flex;align-items:center;justify-content:center;opacity:0;transition:opacity .18s; }
.photo-overlay svg { width:20px;height:20px;color:#fff; }
.photo-drop-zone { flex:1;border:2px dashed var(--c-border2);border-radius:var(--radius);padding:14px 16px;cursor:pointer;transition:all .18s;background:var(--c-surface); }
.photo-drop-zone:hover,.photo-drop-zone.dragging { border-color:var(--c-accent);background:var(--c-accent-lt); }
.dz-content { display:flex;flex-direction:column;gap:4px; }
.dz-icon { width:22px;height:22px;color:var(--c-text3);margin-bottom:4px; }
