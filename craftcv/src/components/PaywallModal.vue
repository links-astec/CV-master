    ...
    <Transition name="paywall-fade">
      <div v-if="show" class="paywall-backdrop" @click.self="$emit('close')">
        <div class="paywall-modal">
          ...
          <!-- ✅ Success state — stays until user clicks Done -->
          ...
          <template v-else>
            <div class="paywall-head">
              ...
              <h2 class="paywall-title">{{ demoMode ? 'Try Demo Export' : 'Export Your CV' }}</h2>
              <p class="paywall-sub">{{ demoMode ? 'Enter your email and we\'ll send your CV as a PDF.' : 'Your CV will be emailed as a PDF to ' }}<strong v-if="!demoMode">{{ userEmail }}</strong></p>
            </div>

            <!-- Demo mode: email input shown prominently -->
            <div v-if="demoMode" class="demo-email-section">
              <div class="f-lbl">Your email address</div>
              <input
                class="f-inp demo-email-inp"
                v-model="demoEmail"
                type="email"
                placeholder="your@email.com"
                @keydown.enter="sendDemo"
                autofocus
              />
              <button class="btn-pay" @click="sendDemo" :disabled="sending || !demoEmail.trim() || !isValidEmail(demoEmail)">
                <svg v-if="sending" class="spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="width:16px;height:16px;"><path d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" opacity=".2"/><path d="M21 12a9 9 0 00-9-9"/></svg>
              ...
              <div class="paywall-secure">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="width:12px;height:12px;"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
            ...
            <template v-else>
              ...
              <div class="paywall-secure">
                ...
                Secured by Stripe · PDF sent to {{ userEmail }}
              </div>
            </template>
          </template>
        </div>
...
const previewScalerStyle = computed(() => ({
  width: '700px', zoom: String(previewScale.value), flexShrink: '0',
}))
...
async function sendCvEmail(overrideEmail) {
  sending.value    = true
  ...
  try {
    ...
    sending.value = false
    sent.value    = true
    // Note: we do NOT emit 'paid' here — let user click Done themselves
  } catch (e) {
...
// Demo mode: send immediately with typed email
async function sendDemo() {
  if (!demoEmail.value.trim() || !isValidEmail(demoEmail.value)) return
  await sendCvEmail(demoEmail.value.trim())
}
...
async function pay() {
  ...
  try {
    ...
    if (data.demo) {
      // Switch to demo mode UI — user must type their email
      demoMode.value  = true
      loading.value   = false
    } else if (data.url) {
...
.paywall-modal{background:var(--c-surface);border-radius:20px;padding:28px 26px;width:100%;max-width:380px;box-shadow:0 24px 64px rgba(0,0,0,.2);position:relative;}
...
.demo-email-inp{font-size:15px;padding:12px 14px;}
...
.paywall-secure{display:flex;align-items:center;justify-content:center;gap:6px;font-size:11.5px;color:var(--c-text3);}
